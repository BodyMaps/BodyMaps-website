...
In another terminal, `VITE_API_BASE=http://127.0.0.1:5001 npm run dev`, then open `/tests/browser/anatomy.html`. No authentication or inference jobs are needed. Use a separate local API port if 5001 is already occupied, updating the harness/fixture asset URL accordingly.
