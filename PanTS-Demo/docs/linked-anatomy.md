...
The existing MPR layout is preserved: axial top left, sagittal top right, coronal bottom left, and 3D bottom right. CT-plane controls are a collapsed popover in the existing 3D Meshes/Volume toolbar, using the viewer’s existing styling.
...
- The actual `/case/fixture` application route was also checked with real CT, segmentation and meshes: pane positions, controls in the existing toolbar, linked slice movement, all-three mode, oblique tilt with zero pivot drift, reset, and switching between Volume and Meshes. Review screenshots use this full application page, not the isolated renderer harness.
